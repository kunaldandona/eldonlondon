...
<p><?php
	...
	printf(
		__( 'Hello %1$s (not %1$s? <a class="brand-color" href="%2$s">Log out</a>)', 'stockie' ), 
...
<div class="message-box">
	<?php
		echo sprintf( esc_attr__( 'From your account dashboard you can view your %1$srecent orders%2$s, manage your %3$sshipping and billing addresses%2$s and %4$sedit your password and account details%2$s.', 'stockie' ), '<a class="brand-color" href="' . esc_url( wc_get_endpoint_url( 'orders' ) ) . '">', '</a>', '<a href="' . esc_url( wc_get_endpoint_url( 'edit-address' ) ) . '">', '<a class="brand-color" href="' . esc_url( wc_get_endpoint_url( 'edit-account' ) ) . '">' );
	?>
</div>
