...
function get_slides($size = 'shop_single', $zoom = true) {
    ...
    foreach ( $attachment_ids as $attachment_id ) {
        ...
        $html .= '<div class="image-wrap woocommerce-product-gallery__image'.esc_attr($with_zoom_class).'" style="position:relative"><div class="woo_c-product-image-slider-trigger btn-round grid-item gallery-image" data-gallery-item="'.$loop.'" data-lazy-item="true"><i class="ion ion-md-expand"></i></div><img class="gimg" src="'.wp_get_attachment_image_url( $attachment_id, $size ).'" alt="'.esc_attr($post->post_title).'"></div>';
        $loop++;
    }
